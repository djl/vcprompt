...
def systems():
    ...
    for system in SYSTEMS:
        name = getattr(system, 'vcprompt_name', system.__name__)
        desc = getattr(system, 'vcprompt_description', '')
...
def vcs(name, description):
    ...
    def wrapped(function):
        function.vcprompt_name = name
        function.vcprompt_description = description
...
def hg(path, string, unknown):
    ...
    # this is probably far more complicated than it needs to be
    if re.search('%(r|h)', string):
        cache_files = map(lambda x: os.path.join(file, '%s.cache' % x),
                          ['branchheads', 'tags'])
        cache_files = filter(os.path.exists, cache_files)
        if cache_files:
            cache_file = cache_files[0]
            with open(cache_file, 'r') as f:
                for line in f:
                    hash, revision = line.strip().split()
                    # tags.cache file format is (revid, hash)
                    # branchheads.cache is (hash, revid)
                    if 'tags.cache' in cache_file:
                        hash, revision = revision, hash
                    hash = hash[:7]
                    break
    ...
    if '%b' in string:
        ...
        # newer versions of Mercurial don't create the '.hg/branch*' files
        # on running 'hg init', but the default branch is named 'default'
        if not os.path.exists(file):
            branch = 'default'
        else:
            with open(file, 'r') as f:
